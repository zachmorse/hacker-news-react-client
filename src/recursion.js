//
// this is the recursion code for parsing and displaying comments from the json data
...
const Comment = ({ level, user, content, time_ago, comments }) => (
  <div class={`comment level-${level}`}>
    <h2>
      <span class="has-text-weight-semibold">{user}</span> // {time_ago}
    </h2>
...
const App = ({ data }) => (
  <div>
    ...
    <h3>Comments ({data.comments_count})</h3>
    <CommentsList comments={data.comments} />
  </div>
);

render(<App data={results} />, document.getElementById("root"));
